...
buildall()
{
	...
	if [[ "${1}" = "yes" ]]; then
		#C++
		mkdir -p $DST_DIR/cpp
		protoc -I=$SRC_DIR --cpp_out=$DST_DIR/cpp/ $SRC_DIR/YouMai.*.proto
		#JAVA
		mkdir -p $DST_DIR/java
		protoc -I=$SRC_DIR --java_out=$DST_DIR/java/ $SRC_DIR/YouMai.*.proto
		#PYTHON
		mkdir -p $DST_DIR/python
		protoc -I=$SRC_DIR --python_out=$DST_DIR/python/ $SRC_DIR/YouMai.*.proto
	else
		#C++
		mkdir -p $DST_DIR/cpp
